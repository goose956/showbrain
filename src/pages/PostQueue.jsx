...
import {
  ...
  Sparkles, Loader2, Send, Plus, Zap,
...
export default function PostQueue({ episodes }) {
  ...
  useEffect(() => {
    apiFetch('/api/posts').then(r => r.json()).then(setPosts).catch(() => {});
  ...
  const handleUpdate = async (updated) => {
    ...
    try {
      await apiFetch(`/api/posts/${updated.id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id) => {
    ...
    try { await apiFetch(`/api/posts/${id}`, { method: 'DELETE' }); } catch {}
  };

  const handleGenerated = async (newPosts) => {
  ...
  const autoDrafts = posts.filter(p => p.autoGenerated && p.status === 'draft');
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {autoDrafts.length > 0 && (
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 bg-th-accent/10 border-b border-th-accent/20 shrink-0">
          <div className="flex items-center gap-2">
            <Zap size={13} className="text-th-accent shrink-0" />
            <p className="text-th-accent text-xs font-medium">
              {autoDrafts.length} auto-generated draft{autoDrafts.length !== 1 ? 's' : ''} ready to review
            </p>
          </div>
          <button
            onClick={() => { setStatusFilter('draft'); setSelectedId(autoDrafts[0].id); }}
            className="text-xs text-th-accent hover:text-th-tx1 underline underline-offset-2 shrink-0"
          >
            Review
          </button>
        </div>
