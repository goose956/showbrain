...
const STATUSES = [
  { id: 'all', label: 'All' },
  { id: 'draft', label: 'Draft', icon: FileEdit, color: 'text-zinc-400 bg-zinc-800 border-zinc-700' },
...
function PlatformIcon({ platform, size = 13 }) {
  ...
  return <Icon size={size} className={colorMap[meta.color] || 'text-zinc-400'} />;
}
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-white font-semibold">Generate posts with AI</h2>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Episode</label>
            <select
              value={episodeId}
          ...
          <div>
            <label className="text-xs text-zinc-500 font-medium mb-1.5 block">Platforms</label>
              ...
              {PLATFORMS.filter((p) => p.id !== 'all').map(({ id, label, icon: Icon, color }) => {
                ...
                return (
                  <button
                    key={id}
                    onClick={() => toggle(id)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border text-sm transition-colors ${active ? (colorMap[color] || 'border-violet-500/50 bg-violet-500/10 text-white') : 'border-zinc-700 text-zinc-500 hover:text-zinc-300 hover:border-zinc-600'}`}
                  >
                    {Icon && <Icon size={13} />}
                    {label}
                  </button>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm transition-colors"
          >
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-3 border-b border-zinc-800 shrink-0">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <p className="text-zinc-600 text-[11px]">From episode</p>
            <p className="text-zinc-400 text-xs truncate">{post.episodeTitle}</p>
          </div>
          {post.episodeUrl && (
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 text-[11px] text-zinc-500 hover:text-violet-300 bg-zinc-800 hover:bg-violet-500/10 border border-zinc-700 hover:border-violet-500/30 px-2.5 py-1.5 rounded-lg transition-colors"
              >
                <Link size={11} />
                Insert link
              </button>
              {trackingLink ? (
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-[11px] text-zinc-500 hover:text-emerald-300 bg-zinc-800 hover:bg-emerald-500/10 border border-zinc-700 hover:border-emerald-500/30 px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50"
      ...
      <div className="flex-1 flex flex-col px-5 py-4 min-h-0">
        <textarea
          ...
          className="flex-1 w-full bg-zinc-800/50 border border-zinc-700 rounded-xl p-4 text-white text-sm leading-relaxed resize-none focus:outline-none focus:border-violet-500 transition-colors"
          placeholder="Post copy…"
        />

        {cc && (
          <div className={`flex justify-end mt-2 text-xs tabular-nums ${cc.over ? 'text-red-400' : 'text-zinc-600'}`}>
            {cc.count} / {cc.limit}
          </div>
        )}

        {hint && (
          <p className="text-zinc-600 text-xs mt-3 leading-relaxed border-t border-zinc-800 pt-3">{hint}</p>
        )}
      </div>
      ...
      <div className="px-5 py-4 border-t border-zinc-800 space-y-2 shrink-0">
        ...
        {isDirty && (
          <button
            onClick={() => save()}
            className="w-full py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white text-sm transition-colors"
          >
            Save changes
          </button>
        )}
        <div className="flex gap-2">
          {demoteLabel && (
            <button
              onClick={demote}
              className="flex-1 py-2 rounded-lg border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-600 text-sm transition-colors"
            >
              {demoteLabel}
            </button>
          )}
          {promoteLabel && (
            <button
              onClick={promote}
              disabled={publishing}
              className={`flex-1 py-2 rounded-lg text-white text-sm transition-colors flex items-center justify-center gap-1.5 disabled:opacity-60 ${
                post.status === 'queued' ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-violet-600 hover:bg-violet-500'
              }`}
            >
      ...
      <div className="flex flex-col w-80 shrink-0 border-r border-zinc-800 overflow-hidden">
        ...
        <div className="px-4 pt-4 pb-3 border-b border-zinc-800 shrink-0">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-white font-semibold text-sm">Post Queue</h1>
            <button
              onClick={() => setShowGenerate(true)}
              className="flex items-center gap-1.5 text-xs text-violet-300 hover:text-white bg-violet-600/20 hover:bg-violet-600/40 border border-violet-500/30 px-2.5 py-1.5 rounded-lg transition-colors"
            >
              <Plus size={12} />
              Generate
            </button>
            ...
            {STATUSES.map(({ id, label }) => (
              <button
                ...
                className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${
                  statusFilter === id ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                {label}
                {id !== 'all' && counts[id] > 0 && (
                  <span className="ml-1 text-zinc-500">{counts[id]}</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center h-40 text-zinc-600 text-xs text-center px-6">
          ...
          {filtered.map((post) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3.5 border-b border-zinc-800/60 transition-colors ${
                  isSelected ? 'bg-violet-600/10 border-l-2 border-l-violet-500' : 'hover:bg-zinc-900/60'
                }`}
              >
                ...
                <p className="text-zinc-300 text-xs leading-relaxed line-clamp-2 mb-1.5">
                  {post.content}
                </p>
                <p className="text-zinc-600 text-[11px] truncate">{post.episodeTitle}</p>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex-1 overflow-hidden bg-zinc-950">
        {selected ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            <FileEdit size={32} className="text-zinc-800 mb-3" />
            <p className="text-zinc-600 text-sm mb-1">Select a post to edit</p>
            <p className="text-zinc-700 text-xs">or generate new posts from an episode</p>
            <button
              onClick={() => setShowGenerate(true)}
              className="mt-5 flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-600/20 hover:bg-violet-600/30 border border-violet-500/30 text-violet-300 hover:text-white text-sm transition-colors"
            >
              <Sparkles size={14} />
              Generate posts
            </button>
